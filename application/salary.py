...
class rumonth:
    def __init__(self, year, month, day):
        ...
        current_day = rudate(year=year, month=month, day=1)
    ...
    def self_calculate(self, base_pay, accuracy=2):
        if self.norm_days == 0:
            return 0
        else:
...
def calcilate_salary(person, start_data, end_data):
    ...
    return sum(
        [month.calculate(base_pay) for month in calculated_monthes]
    )
